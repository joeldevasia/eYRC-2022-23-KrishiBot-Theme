...
* Theme: Krishi Bot (KB)
* Functions: [ bot_status_updater, tf_listener_and_broadcaster, get_fruit, main, slowstart, turnLeft, stop,  smallLeft, smallRight, control_loop ]
"""
...
def tf_listener_and_broadcaster(tf_listener):
    """
    * Function Name: tf_listener_and_broadcaster
    * Input: tf_listener
    * Output: None
    * Logic: This function is used to get the transform data from the robot
    *
    * Example Call: tf_listener_and_broadcaster(tf_listener)
    """
    ...
    try:
        ...
        if (
            tf_listener.canTransform(
                "/ebot_base", "/yellow_bell_pepper", rospy.Time(0))
            == True
            or tf_listener.canTransform("/ebot_base", "/red_bell_pepper", rospy.Time(0))
            == True
        ):
                ...
                if (
                    tf_listener.canTransform(
                        ...
                        tf_listener.getLatestCommonTime(
                            "/ebot_base", "/yellow_bell_pepper"
                        ),
                    )
                    == True
                ):
                    trans_yellow = tf_listener.lookupTransform(
                        "/ebot_base", "/yellow_bell_pepper", rospy.Time(0)
                    )
                    ...
                    global yellow_Cordinates
                    yellow_Cordinates = [
                        round(trans_yellow[0][0], 3),
                        round(trans_yellow[0][1], 3),
                        round(trans_yellow[0][2], 3),
                        0,
                        0,
                        0,
                    ]
            ...
            try:
                if (
                    tf_listener.canTransform(
                        ...
                        "/red_bell_pepper",
                        tf_listener.getLatestCommonTime(
                            "/ebot_base", "/red_bell_pepper"
                        ),
                    )
                    == True
                ):
                    trans_red = tf_listener.lookupTransform(
                        "/ebot_base", "/red_bell_pepper", rospy.Time(0)
                    )
                    ...
                    global red_Cordinates
                    red_Cordinates = [
                        round(trans_red[0][0], 3),
                        round(trans_red[0][1], 3),
                        round(trans_red[0][2], 3),
                        0,
                        0,
                        0,
                    ]
...
class Ur5Moveit:
    def __init__(self):
        # keeping it as default state in zero, as it acknowledges the topic connection
        self.ack_val = 0
        self.ack_val_poseJoint = 0  # for pose joint val ack
        self._eef_link = (
            "wrist_3_link"  # declaring end effector link for the clas manually
        )

        # Subscriber nodes
        rospy.Subscriber("/joint_pose_ack", Bool, self.ack_clbck, queue_size=1)
        rospy.Subscriber("/joint_ee_pose", String,
                         self.pose_joint_clbck, queue_size=1)

        # acknowledgment log on terminal
        rospy.logwarn(
            "Make sure that the Manipulation Help node initialization is complete."
        )
        rospy.loginfo(
            "!!!!!!!!!!!!!!!! UR5_Moveit Initialization complete !!!!!!!!!!!!!!!!!"
        )

        # Publisher variable for joint angles
        self.pub_joint_angles = rospy.Publisher(
            "/set_joint_value_target_wait_topic", Float32MultiArray, queue_size=10
        )

        # Publisher variable for pose values
        self.pub_pose_values = rospy.Publisher(
            "/set_pose_value_target_wait_topic", Pose, queue_size=10
        )

        # Publisher variable for requesting pose of ee and joint values
        self.req_pose_joint = rospy.Publisher(
            "/joint_ee_pose_req", Empty, queue_size=1)
        rospy.sleep(1)
    ...
    def pose_joint_clbck(self, msg):
        ...
        """
        self.pose_and_joint_angles = eval(
            msg.data
        )  # storing the data part (expected: String)
        # and evaling for python data types
        self.sample_pose = (
            Pose()
        )  # creating Pose instance for converting manually the data
        ...
        self.pose_and_joint_angles[
            0
        ] = self.sample_pose  # Replacing the first element with actual pose format
    ...
    def set_joint_angles(self, arg_list_joint_angles):
        ...
        data_for_float32 = (
            Float32MultiArray()
        )  # Float32Array data type has two sub-data types
        data_for_float32.data = arg_list_joint_angles  # storing in data part

        self.pub_joint_angles.publish(
            data_for_float32
        )  # publishes joint angles on /set_joint_value_target_wait_topic
        rospy.wait_for_message(
            "/joint_pose_ack", Bool, timeout=None
        )  # wait till acknowledgment is recieved
    ...
    def print_pose_ee_joint(self):  # but outputs both pose and joint angles
        ...
        # Publishing as signal to request print the pose and joint angles
        self.req_pose_joint.publish()
        rospy.wait_for_message(
            "/joint_ee_pose", String, timeout=None
        )  # wait till acknowledgment is recieved
        ...
        try:
            ...
            pose_values = self.pose_and_joint_angles[
                0
            ]  # Storing the first part of the list
            ...
            (roll, pitch, yaw) = euler_from_quaternion(
                quaternion_list
            )  # converting the quaternion to euler
            ...
            rospy.logerr(
                "Exception occurred while executing print function: ", str(e))
            self.pose_and_joint_angles = [
                self.sample_pose,
                [0, 0, 0, 0, 0, 0],
    ...
    def gripper_control(self, logic_level):
        ...
        )  # wait for service availabilty
        spawn_srv = rospy.ServiceProxy(
            "/ur_hardware_interface/set_io", SetIO
        )  # proceed to defining proxy service
        spawn_srv(
            fun=1, pin=16, state=logic_level
        )  # passing the logic to pin 16 (gripper connection)
        rospy.sleep(4)  # wait as it non-blocking call
        rospy.logdebug(
            "Gripper control executed successfully: " + str(logic_level))
...
def get_fruit(fruit_cordinates, drop_angles, fruit_Name, scan, tf_listener):
    ...
    * Input: fruit_cordinates, drop_angles, fruit_Name, scan, tf_listener
    * Output: None
    * Logic: This function is used to get the fruit from the plant and place it in the basket.
    *
    * Example Call: get_fruit(fruit_cordinates, drop_angles, fruit_Name, scan, tf_listener)
    """
    global ur5
    ...
    while x == False and y == False and z == False:
        ...
        x = (
            True
            if (round(currentPose[0].position.x, 2) == round(temp[0], 2))
            else False
        )
        y = (
            True
            if (round(currentPose[0].position.y, 2) == round(temp[1], 2))
            else False
        )
        z = (
            True
            if (round(currentPose[0].position.z, 2) == round(temp[2], 2))
            else False
        )

    temp[1] = temp[1] + 0.3  # finally equals to temp - 0.25
    position.y = temp[1]
    pose_goal.position = position
...
def main():
    ...
    print("Started")
    ur5.set_joint_angles(lst_joint_angles_scan_left)
    # rospy.sleep(7)
    ...
    while not rospy.is_shutdown():
        ...
        if (
            len(yellow_Cordinates) != 0 or len(red_Cordinates) != 0
        ) and bot_Status == "stop":
            try:
        ...
        bot_pub.publish("ready")
        print("bot_status ManiStack While: ", bot_Status)
